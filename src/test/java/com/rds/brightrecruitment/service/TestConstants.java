package com.rds.brightrecruitment.service;

public class TestConstants {

  public static final Long BOOK_ID = 1L;
  public static final String BOOK_TITLE = "Invincible";
  public static final String BOOK_AUTHOR = "Philip Dick";
  public static final String BOOK_ISBN = "978-83-7432-357-4";
  public static final Integer BOOK_NUMBER_OF_PAGES = 402;
  public static final Integer BOOK_RATING = 5;
  public static final String COMMENT_CONTENT = "Nice book bro";

}
